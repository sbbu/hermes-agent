...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a GitHub PR URL appears in a recent comment — a prior worker already opened a PR). A later explicit requeue/recovery event (`status`, `promoted`, `unblocked`, `reclaimed`, crash/timeout/stale recovery, or rate-limit recovery) overrides older success/PR evidence so dependency-gated and recovered work can continue. Otherwise the task stays in `ready` and the next tick checks again. |
