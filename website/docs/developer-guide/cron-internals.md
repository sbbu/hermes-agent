...
**Named channels** (`slack:#engineering`, `discord:#engineering`, or a friendly name like `slack:engineering`) are resolved against the channel directory the gateway builds from connected adapters, so the gateway must have discovered the channel for name resolution to succeed; raw IDs (`slack:C0123ABCD45`) always work. Names shared by multiple destinations are rejected instead of selecting one arbitrarily. Use the target emitted by `hermes send --list`: duplicate Discord names are guild-qualified when possible, and otherwise the list falls back to raw IDs. For platforms with unconstrained opaque IDs, the list emits `name=<friendly label>` when name resolution is intended; unprefixed values remain raw IDs so a display-name collision cannot redirect delivery. Existing jobs created from an older list must add `name=` to friendly targets on those platforms; use `id=` when a literal raw ID itself begins with `name=`.
