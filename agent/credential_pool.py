...
class CredentialPool:
    ...
    def _persist(
        ...
        *,
        ...
        # Self-locking (RLock): snapshotting and generation updates must not
        # race deferred refreshes or concurrent selection/rotation.
        with self._lock:
            effective_advance = (
        ...
        try:
            ...
            if self.provider == "openai-codex":
                ...
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    # This deferred-refresh path runs outside the pool lock;
                    # keep the read-modify-write and owner-generation update
                    # atomic with every other in-process pool mutation.
                    with self._lock:
                        removed_ids = [
                            item.id for item in self._entries
        ...
        *,
        ...
        with self._lock:
            ...
            if entry is None and api_key_hint:
                ...
                # (another process already rotated), selecting current would
                # identify the next key instead of the failed one.
                entry = next(
            ...
            def mark_failed_entries() -> bool:
                if identity_supplied and failed_runtime_key:
                    failed_entries = [
                        ...
                        if candidate.runtime_api_key == failed_runtime_key
                    ]
                else:
                    failed_entries = [
                        candidate
