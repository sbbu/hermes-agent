...
@pytest.mark.parametrize("include_stable_id", [False, True])
def test_stale_401_cannot_mark_concurrently_reauthed_entry_dead(
    ...
    if include_stable_id:
        rotate_kwargs["credential_id"] = old.id
    selected = pool.mark_exhausted_and_rotate(**rotate_kwargs)

    # A stale failure must never quarantine the refreshed generation. The
    # upstream single-entry unmatched-identity contract now returns None rather
    # than pretending that it rotated, so callers surface/retry the request
    # without handing back the same credential as a successful recovery.
    assert selected is None
    reloaded = pool.entries()
    assert len(reloaded) == 1
    assert reloaded[0].access_token == "fresh-token"
